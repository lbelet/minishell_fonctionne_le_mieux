...
void	ft_get_words_modif(char **tab, const char *str, char c, int *code_caractere)
{
	int	i;
	int	j;
    int k;
	...
	while (i < n)
	{
		...
		while (*str == c && code_caractere[k++] == 6)
			str++;
		while ((*(str + j) && *(str + j) != c) || (*(str + j) == c && code_caractere[k++] != 6))
			j++;
...
char	**ft_split_modif(char const *s, char c, int *code_caractere)
{
	int		n;
	char	**tab;
