...
void ft_check_dollars(char *commande)
{
	int i;
	int nbr_dollars;
	int *code_caractere;
	...
	k = 0;
	nbr_dollars = 0;
	code_caractere = ft_code_caractere_bis(commande);
	str_cmd = "";
	while (commande[i])
	{
		if (commande[i] == '$' && code_caractere[i] != 8)
			nbr_dollars++;
		i++;
	}
	if (nbr_dollars > 0)
	{
		tmp = malloc((nbr_dollars + 1) * sizeof(char *));
		tmp[nbr_dollars] = NULL;
	}
	else
	{
		tmp = NULL;
	...
	while (commande[i])
	{
		while (commande[i] != '$' || (commande[i] == '$' && code_caractere[i] == 8))
		{
			str_cmd = ft_strjoin_modif(str_cmd, commande[i]);
			i++;
		}
		if (commande[i] == '$' && code_caractere[i] != 8 && commande[i + 1])
		{
...
void ft_check_redir(int *fd, char **cmd, char **commande)
{
	...
	while (cmd[i])
	{	
		caractere = ft_code_caractere_bis(cmd[i]);
		j = 0;
		while (cmd[i][j])
		{
...
void ft_one(char **split_pipe, char **envp)
{
	...
	fd[0] = 0;
	fd[1] = 0;
    cmd = ft_split_modif(split_pipe[0], ' ', ft_code_caractere(split_pipe[0]), 0);
	while(cmd[i])
	{
		printf("cmd: %s\n", cmd[i]);
		i++;
